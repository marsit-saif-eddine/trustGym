...
const CoursCard: React.FC<CoursCardProps> = ({ image, name, onEdit, onDelete }) => {
  return (
    <div className="relative p-2 bg-card border border-[hsl(var(--border))] rounded-[12px] shadow-custom-card items-center space-x-4 hover:border-[hsl(var(--primary))] cursor-pointer">
      ...
      <div className="mt-4 mb-2 flex justify-between items-center">
        ...
        <div className="flex space-x-2">
          <Button onClick={onDelete} variant="destructive" size="icon" className="flex items-center justify-center w-7 h-7 bg-[hsl(var(--secondary))] rounded-full">
            <HiOutlineTrash className="text-[hsl(var(--destructive))] w-4 h-4" />
          </Button>
          <Button onClick={onEdit} variant="default" size="icon" className="flex items-center justify-center w-7 h-7 bg-[hsl(var(--secondary))] rounded-full">
